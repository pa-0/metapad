	...
	case WM_COMMAND:
		...
		case ID_DROP_REPLACE:
			...
			if (frDlgId == ID_REPLACE){
				...
				SendDlgItemMessage(hdlgFind, ID_DROP_FIND, WM_GETTEXT, MAXFIND, (WPARAM)szTmp);
				if (!bNoFindHidden) bOkEna &= ParseForEscapeSeqs(szTmp, NULL, NULL);
				l1 = lstrlen(szTmp);
				SendDlgItemMessage(hdlgFind, ID_DROP_REPLACE, WM_GETTEXT, MAXFIND, (WPARAM)szTmp);
				if (!bNoFindHidden) bOkEna &= ParseForEscapeSeqs(szTmp, NULL, NULL);
				l2 = lstrlen(szTmp);
				bOkEna &= l1 > l2;
			...
			switch (frDlgId){
				case ID_REPLACE:
					for (l1 = 0; l1 < NUMFINDS; l1++) {
						SendDlgItemMessage(hdlgFind, ID_DROP_REPLACE, CB_GETLBTEXT, l1, (WPARAM)szBuf);
				...
				case ID_INSERT_TEXT:
					for (l1 = 0; l1 < NUMINSERTS; l1++) {
						SendDlgItemMessage(hdlgFind, ID_DROP_INSERT, CB_GETLBTEXT, l1, (WPARAM)szBuf);
...
BOOL CALLBACK Advanced2PageProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam) {
	...
	case WM_COMMAND:
		...
		case IDC_BUTTON_BROWSE:
			if (BrowseFile(hwndDlg, _T("dll"), NULL, _T("metapad language plugins (*.dll)\0*.dll\0All Files (*.*)\0*.*\0"), FALSE, FALSE, FALSE, &sz)){
...
BOOL CALLBACK GeneralPageProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam) {
	...
	case WM_COMMAND:
		...
		case IDC_BUTTON_BROWSE2:
			if (BrowseFile(hwndDlg, _T("exe"), NULL, _T("Executable Files (*.exe)\0*.exe\0All Files (*.*)\0*.*\0"), FALSE, FALSE, FALSE, &sz))
...
//	options.nStatusFontWidth = LOWORD(GetDialogBaseUnits());
...

	{
		...
		hm = GetModuleHandle(_T("uxtheme.dll"));
		SetWindowTheme = (SWT)(GetProcAddress(hm, "SetWindowTheme"));
	}
